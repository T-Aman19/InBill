...
impl PgAccess {
    ...
    pub fn stop_db_command_sync(&self, database_dir: &PathBuf) -> PgCommandSync {
        ...
        command
            .get_mut()
            .args(&["stop", "-m", "fast", "-w", "-D", database_dir.to_str().unwrap()]);
        // Prevent a console window from flashing on Windows when called from Drop.
        #[cfg(target_os = "windows")]
        {
            use std::os::windows::process::CommandExt;
            command.get_mut().creation_flags(0x08000000); // CREATE_NO_WINDOW
        }
        command
    }
}
