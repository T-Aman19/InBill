...
export default function BillingPage() {
    ...
    const [mobilePane, setMobilePane] = useState("receipt");
    ...
    async function handleLinkCustomer() {
        ...
        setCustErr("");
        try {
            const cust = await api.customers.upsert({ phone, name: custName.trim() || undefined });
    ...
    const { data: custMatches = [] } = useQuery({
        ...
        queryFn: () => api.customers.search(custSearch),
    ...
    const removeChargeMutation = useMutation({
        mutationFn: (lineId) => api.bills.removeCharge(billId, lineId),
        onSuccess: () => qc.invalidateQueries({ queryKey: ["bill", billId] }),
    });
    ...
    if (bill.isPaid)
        return (_jsxs("div", { style: { flex: 1, display: "flex", flexDirection: isTablet ? "column" : "row", overflow: isTablet ? "auto" : "hidden" }, children: [_jsx("div", { className: "scroll print-receipt", style: { flex: 1, padding: isTablet ? "20px 20px" : "32px 40px", background: "var(--color-bg)" }, children: _jsxs("div", { style: { maxWidth: 540, margin: "0 auto" }, children: [_jsxs("div", { style: { textAlign: "center", paddingBottom: 18, borderBottom: "1px dashed var(--color-line-strong)" }, children: [_jsx("div", { className: "display", style: { fontSize: 18, fontWeight: 700 }, children: outlet?.name ?? "InBill" }), outlet?.address && _jsx("div", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 2 }, children: outlet.address }), outlet?.gstin && _jsxs("div", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 2 }, children: ["GSTIN ", outlet.gstin] }), outlet?.fssaiNumber && _jsxs("div", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 2 }, children: ["FSSAI ", outlet.fssaiNumber] })] }), _jsxs("div", { style: { display: "flex", justifyContent: "space-between", padding: "14px 0", fontSize: 12, color: "var(--color-ink-3)" }, children: [_jsxs("span", { children: ["Bill #", bill.billNumber] }), _jsx("span", { children: formatBillTime(bill.createdAt) })] }), bill.items && bill.items.length > 0 && (_jsxs("div", { style: { borderTop: "1px solid var(--color-line)", borderBottom: "1px solid var(--color-line)" }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", padding: "10px 0", fontSize: 11, color: "var(--color-ink-3)", letterSpacing: ".04em", textTransform: "uppercase", fontWeight: 500 }, children: [_jsx("span", {}), _jsx("span", { children: "Item" }), _jsx("span", { style: { textAlign: "center" }, children: "Qty" }), _jsx("span", { style: { textAlign: "right" }, children: "Amount" })] }), bill.items.map((l, i) => (_jsxs("div", { style: { borderTop: "1px solid var(--color-line)", padding: "10px 0" }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", fontSize: 14, alignItems: "center" }, children: [_jsx("span", { className: `veg-dot ${l.isVeg ? "veg" : "nonveg"}`, style: { width: 10, height: 10 } }), _jsx("span", { style: { color: "var(--color-ink)", fontWeight: 500 }, children: l.name }), _jsx("span", { style: { textAlign: "center", fontFamily: "var(--font-mono)", color: "var(--color-ink-2)" }, children: l.quantity }), _jsx("span", { style: { textAlign: "right", fontFamily: "var(--font-mono)", fontWeight: 500 }, children: formatCurrency(lineTotal(l)) })] }), (l.modifiers ?? []).map((m, mi) => (_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", fontSize: 12, color: "var(--color-ink-3)", marginTop: 4 }, children: [_jsx("span", {}), _jsxs("span", { style: { paddingLeft: 4 }, children: ["+ ", m.name] }), _jsx("span", {}), _jsx("span", { style: { textAlign: "right", fontFamily: "var(--font-mono)" }, children: formatCurrency(Number(m.price) * l.quantity) })] }, mi)))] }, i)))] })), _jsxs("div", { style: { paddingTop: 16 }, children: [_jsx(Row, { label: "Subtotal", value: formatCurrency(bill.subtotal) }), (bill.discountLines ?? []).length > 0
                                        ? (bill.discountLines ?? []).map((line, i) => _jsx(Row, { label: line.label, value: "− " + formatCurrency(line.amount), dim: true }, i))
                                        : Number(bill.discountAmount) > 0 && _jsx(Row, { label: "Discount", value: "− " + formatCurrency(bill.discountAmount), dim: true }), bill.taxLines.map((line, i) => (_jsx(Row, { label: `${line.name} (${line.rate}%)`, value: formatCurrency(line.amount), dim: true }, i))), (bill.chargeLines ?? []).map((line, i) => _jsx(Row, { label: line.label, value: formatCurrency(line.amount) }, i)), _jsx("div", { style: { height: 1, background: "var(--color-line-strong)", margin: "12px 0" } }), _jsx(Row, { label: "Total", value: formatCurrency(bill.total), big: true })] }), bill.payments.length > 0 && (_jsxs("div", { style: { marginTop: 20 }, children: [bill.payments.length > 1 && (_jsx("div", { style: { display: "flex", borderRadius: 6, overflow: "hidden", height: 6, marginBottom: 10 }, children: bill.payments.map((p) => {
                                            ...
                                            return _jsx("div", { style: { flex: `0 0 ${pct}%`, background: bg } }, p.id);
                                        }) })), _jsx("div", { style: { display: "flex", gap: 14, flexWrap: "wrap" }, children: bill.payments.map((p) => {
                                            const bg = p.mode === "cash" ? "var(--color-amber)" : p.mode === "card" ? "var(--color-blue)" : "var(--color-green)";
                                            return (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 5, fontSize: 12 }, children: [_jsx("div", { style: { width: 8, height: 8, borderRadius: 2, background: bg, flexShrink: 0 } }), _jsx("span", { style: { textTransform: "capitalize", color: "var(--color-ink-3)" }, children: p.mode }), _jsx("span", { style: { fontFamily: "var(--font-mono)", color: "var(--color-ink-2)", fontWeight: 500 }, children: formatCurrency(p.amount) })] }, p.id));
                                        }) })] })), _jsxs("div", { style: { textAlign: "center", marginTop: 28, paddingTop: 20, borderTop: "1px dashed var(--color-line)", fontSize: 11, color: "var(--color-ink-4)" }, children: ["Thank you for visiting", outlet?.name ? ` ${outlet.name}` : "", "!"] })] }) }), _jsxs("div", { className: "animate-slide-right", style: {
                        ...
                        overflowY: isTablet ? "visible" : "auto",
                    }, children: [_jsx("div", { className: "eyebrow", style: { color: "var(--v2-marigold)" }, children: "Payment received" }), _jsx("div", { className: "animate-pop", style: {
                                ...
                                marginTop: 28, marginBottom: 20, flexShrink: 0,
                            }, children: _jsx("svg", { width: "38", height: "38", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.4", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M5 12l5 5L20 7" }) }) }), _jsx("div", { className: "display", style: { fontSize: 42, fontWeight: 700, lineHeight: 1.1, color: "#fff", letterSpacing: "-.02em" }, children: formatCurrency(bill.total) }), _jsxs("div", { style: { fontSize: 13, color: "oklch(100% 0 0 / .45)", marginTop: 8 }, children: ["Bill #", bill.billNumber, " \u00B7 ", formatBillTime(bill.createdAt)] }), bill.payments.length > 0 && (_jsx("div", { style: { marginTop: 24, padding: "14px 16px", background: "oklch(100% 0 0 / .04)", borderRadius: 12, border: "1px solid oklch(100% 0 0 / .08)", display: "flex", flexDirection: "column", gap: 6 }, children: bill.payments.map((p) => (_jsxs("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 13 }, children: [_jsx("span", { style: { textTransform: "capitalize", color: "oklch(100% 0 0 / .55)" }, children: p.mode }), _jsx("span", { style: { fontFamily: "var(--font-mono)", color: "#fff", fontWeight: 500 }, children: formatCurrency(p.amount) })] }, p.id))) })), loyaltyInfo && (_jsxs("div", { style: { marginTop: 14, padding: "14px 16px", background: "oklch(70% 0.17 55 / .08)", borderRadius: 12, border: "1px solid oklch(70% 0.17 55 / .2)" }, children: [_jsx("div", { style: { fontSize: 12, color: "var(--v2-marigold)", fontWeight: 600, marginBottom: 4 }, children: loyaltyInfo.customer.name ?? loyaltyInfo.customer.phone }), _jsx("div", { style: { fontSize: 12, color: "oklch(100% 0 0 / .45)" }, children: loyaltyInfo.program
                                        ? _jsxs(_Fragment, { children: ["+", loyaltyInfo.pointsToEarn, " pts earned \u00B7 ", loyaltyInfo.totalPoints, " total \u00B7 ", loyaltyInfo.tier] })
                                        : loyaltyInfo.customer.name ? loyaltyInfo.customer.phone : "Customer" })] })), _jsx("div", { style: { flex: 1, minHeight: 32 } }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 10 }, children: [_jsxs("button", { onClick: () => triggerPrint(), style: {
                                    ...
                                    }, children: ["Back to floor", _jsx("svg", { width: "18", height: "18", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M5 12h14M13 6l6 6-6 6" }) })] }), _jsxs("div", { style: { textAlign: "center", fontSize: 12, color: "oklch(100% 0 0 / .3)", marginTop: 2 }, children: ["Auto-returns in ", Math.floor(countdown / 60), ":", String(countdown % 60).padStart(2, "0")] })] })] })] }));
    return (_jsxs("div", { style: { flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--color-bg)" }, children: [_jsxs("div", { style: {
                    ...
                    { id: "payment", label: "Payment" },
                        ...
                        fontSize: 13, fontWeight: mobilePane === p.id ? 600 : 500, cursor: "pointer", fontFamily: "inherit",
                    }, children: p.label }, p.id))) })), _jsxs("div", { style: { flex: 1, display: "grid", gridTemplateColumns: isTablet ? "1fr" : "1fr 440px", overflow: "hidden" }, children: [_jsx("div", { className: "scroll print-receipt", style: { padding: "24px 32px", display: isTablet && mobilePane !== "receipt" ? "none" : "block" }, children: _jsxs("div", { style: { maxWidth: 560, margin: "0 auto" }, children: [_jsxs("div", { style: { textAlign: "center", paddingBottom: 18, borderBottom: "1px dashed var(--color-line-strong)" }, children: [_jsx("div", { style: { fontSize: 16, fontWeight: 600 }, children: outlet?.name ?? "InBill" }), outlet?.address && _jsx("div", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 2 }, children: outlet.address }), outlet?.gstin && _jsxs("div", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 2 }, children: ["GSTIN ", outlet.gstin] })] }), _jsxs("div", { style: { display: "flex", justifyContent: "space-between", padding: "14px 0", fontSize: 12, color: "var(--color-ink-3)" }, children: [_jsxs("span", { children: ["Bill #", bill.billNumber] }), _jsx("span", { children: formatBillTime(bill.createdAt) })] }), bill.items && bill.items.length > 0 && (_jsxs("div", { style: { borderTop: "1px solid var(--color-line)", borderBottom: "1px solid var(--color-line)" }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", padding: "10px 0", fontSize: 11, color: "var(--color-ink-3)", letterSpacing: ".04em", textTransform: "uppercase", fontWeight: 500 }, children: [_jsx("span", {}), _jsx("span", { children: "Item" }), _jsx("span", { style: { textAlign: "center" }, children: "Qty" }), _jsx("span", { style: { textAlign: "right" }, children: "Amount" })] }), bill.items.map((l, i) => (_jsxs("div", { style: { borderTop: "1px solid var(--color-line)", padding: "10px 0" }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", fontSize: 14, alignItems: "center" }, children: [_jsx("span", { className: `veg-dot ${l.isVeg ? "veg" : "nonveg"}`, style: { width: 10, height: 10 } }), _jsx("span", { style: { color: "var(--color-ink)", fontWeight: 500 }, children: l.name }), _jsx("span", { style: { textAlign: "center", fontFamily: "var(--font-mono)", color: "var(--color-ink-2)" }, children: l.quantity }), _jsx("span", { style: { textAlign: "right", fontFamily: "var(--font-mono)", fontWeight: 500 }, children: formatCurrency(lineTotal(l)) })] }), (l.modifiers ?? []).map((m, mi) => (_jsxs("div", { style: { display: "grid", gridTemplateColumns: "20px 1fr 52px 100px", fontSize: 12, color: "var(--color-ink-3)", marginTop: 4 }, children: [_jsx("span", {}), _jsxs("span", { style: { paddingLeft: 4 }, children: ["+ ", m.name] }), _jsx("span", {}), _jsx("span", { style: { textAlign: "right", fontFamily: "var(--font-mono)" }, children: formatCurrency(Number(m.price) * l.quantity) })] }, mi)))] }, i)))] })), _jsxs("div", { style: { paddingTop: 16 }, children: [_jsx(Row, { label: "Subtotal", value: formatCurrency(bill.subtotal) }), (bill.discountLines ?? []).length > 0
                                            ? (bill.discountLines ?? []).map((line, i) => (_jsx(Row, { label: line.label, value: "− " + formatCurrency(line.amount), dim: true }, i)))
                                            : Number(bill.discountAmount) > 0 && (_jsx(Row, { label: "Discount", value: "− " + formatCurrency(bill.discountAmount), dim: true })), bill.taxLines.map((line, i) => (_jsx(Row, { label: `${line.name} (${line.rate}%)`, value: formatCurrency(line.amount), dim: true }, i))), (bill.chargeLines ?? []).map((line, i) => _jsx(Row, { label: line.label, value: formatCurrency(line.amount) }, i)), _jsx("div", { style: { height: 1, background: "var(--color-line-strong)", margin: "12px 0" } }), _jsx(Row, { label: "Total", value: formatCurrency(bill.total), big: true })] }), bill.payments.length > 0 && (_jsxs("div", { style: { marginTop: 18, padding: 12, background: "var(--color-blue-soft)", borderRadius: 10, fontSize: 13 }, children: [_jsx("div", { style: { fontSize: 11, color: "var(--color-blue)", fontWeight: 600, letterSpacing: ".04em", textTransform: "uppercase", marginBottom: 6 }, children: "Payments received" }), bill.payments.map((p) => (_jsxs("div", { style: { display: "flex", justifyContent: "space-between", color: "var(--color-ink-2)", padding: "3px 0" }, children: [_jsx("span", { style: { textTransform: "capitalize" }, children: p.mode }), _jsx("span", { style: { fontFamily: "var(--font-mono)" }, children: formatCurrency(p.amount) })] }, p.id))), _jsxs("div", { style: { display: "flex", justifyContent: "space-between", fontWeight: 600, marginTop: 6, paddingTop: 6, borderTop: "1px solid rgba(0,0,0,.06)" }, children: [_jsx("span", { children: "Remaining" }), _jsx("span", { style: { fontFamily: "var(--font-mono)", color: remaining > 0 ? "var(--color-red)" : "var(--color-green)" }, children: formatCurrency(remaining) })] })] }))] }) }), _jsxs("div", { style: { background: "var(--color-surface)", borderLeft: isTablet ? "none" : "1px solid var(--color-line)", padding: 24, display: isTablet && mobilePane !== "payment" ? "none" : "flex", flexDirection: "column", gap: 18, overflowY: "auto" }, children: [_jsx("div", { style: { fontSize: 12, color: "var(--color-ink-3)", letterSpacing: ".06em", textTransform: "uppercase", fontWeight: 500 }, children: "Collect payment" }), !loyaltyInfo && !bill.isPaid && (_jsxs("div", { style: { border: "1px solid var(--color-line)", borderRadius: 12, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }, children: [_jsxs("div", { style: { fontSize: 12, fontWeight: 500, color: "var(--color-ink-2)" }, children: ["Customer ", _jsx("span", { style: { fontWeight: 400, color: "var(--color-ink-3)" }, children: "(optional \u00B7 for loyalty)" })] }), _jsxs("div", { style: { display: "flex", gap: 8 }, children: [_jsx("input", { value: custPhone, onChange: (e) => setCustPhone(e.target.value.replace(/\D/g, "").slice(0, 10)), onKeyDown: (e) => e.key === "Enter" && handleLinkCustomer(), placeholder: "10-digit mobile", inputMode: "numeric", maxLength: 10, style: { flex: 1, minWidth: 0, height: 36, borderRadius: 8, border: "1px solid var(--color-line-strong)", padding: "0 10px", fontSize: 13, background: "var(--color-bg)", color: "var(--color-ink)", outline: "none", fontFamily: "inherit" }, onFocus: (e) => (e.currentTarget.style.borderColor = "var(--color-ink-3)"), onBlur: (e) => (e.currentTarget.style.borderColor = "var(--color-line-strong)") }), _jsx("input", { value: custName, onChange: (e) => setCustName(e.target.value), placeholder: "Name", style: { flex: 1, minWidth: 0, height: 36, borderRadius: 8, border: "1px solid var(--color-line-strong)", padding: "0 10px", fontSize: 13, background: "var(--color-bg)", color: "var(--color-ink)", outline: "none", fontFamily: "inherit" }, onFocus: (e) => (e.currentTarget.style.borderColor = "var(--color-ink-3)"), onBlur: (e) => (e.currentTarget.style.borderColor = "var(--color-line-strong)") })] }), custSearch.length >= 2 && custMatches.length > 0 && (_jsx("div", { className: "scroll", style: { border: "1px solid var(--color-line)", borderRadius: 8, overflow: "hidden", maxHeight: 176, overflowY: "auto" }, children: custMatches.map((cust, i) => (_jsxs("button", { onClick: () => handleSelectCustomer(cust.id), disabled: custLinking, style: { width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "9px 12px", border: "none", borderTop: i === 0 ? "none" : "1px solid var(--color-line)", background: "var(--color-bg)", cursor: custLinking ? "wait" : "pointer", textAlign: "left", fontFamily: "inherit" }, children: [_jsxs("div", { style: { minWidth: 0 }, children: [_jsx("div", { style: { fontSize: 13, fontWeight: 500, color: "var(--color-ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, children: cust.name || "Guest" }), _jsx("div", { style: { fontSize: 12, color: "var(--color-ink-3)", fontFamily: "var(--font-mono)" }, children: cust.phone })] }), typeof cust.loyaltyPoints === "number" && cust.loyaltyPoints > 0 && (_jsxs("span", { style: { fontSize: 11, color: "var(--color-amber)", fontWeight: 600, flexShrink: 0 }, children: [cust.loyaltyPoints, " pts"] }))] }, cust.id))) })), custErr && _jsx("div", { style: { fontSize: 12, color: "var(--color-red)" }, children: custErr }), _jsx("button", { onClick: handleLinkCustomer, disabled: !custPhone.trim() || custLinking, style: { height: 34, borderRadius: 8, border: "1px solid var(--color-line-strong)", background: "var(--color-surface-2)", color: "var(--color-ink-2)", fontSize: 13, fontFamily: "inherit", cursor: custPhone.trim() ? "pointer" : "not-allowed", opacity: custPhone.trim() ? 1 : .5 }, children: custLinking ? "Linking…" : "Add new customer" })] })), (bill.discountLines ?? []).length > 0 && (_jsx("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: (bill.discountLines ?? []).map((line) => (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--color-green-soft)", borderRadius: 8 }, children: [_jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "var(--color-green)", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("path", { d: "M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z" }) }), _jsx("span", { style: { flex: 1, fontSize: 13, color: "var(--color-green)", fontWeight: 500 }, children: line.label }), _jsxs("span", { style: { fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--color-green)", fontWeight: 600 }, children: ["\u2212 ", formatCurrency(line.amount)] }), bill.payments.length === 0 && (_jsx("button", { onClick: () => removeDiscountMutation.mutate(line.id), style: { border: "none", background: "transparent", cursor: "pointer", color: "var(--color-green)", padding: 2, display: "flex", alignItems: "center" }, children: _jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", children: _jsx("path", { d: "M18 6L6 18M6 6l12 12" }) }) }))] }, line.id))) })), (bill.chargeLines ?? []).length > 0 && (_jsx("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: (bill.chargeLines ?? []).map((line) => (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--color-blue-soft)", borderRadius: 8 }, children: [_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "var(--color-blue)", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("circle", { cx: "12", cy: "12", r: "10" }), _jsx("path", { d: "M12 8v4l3 3" })] }), _jsx("span", { style: { flex: 1, fontSize: 13, color: "var(--color-blue)", fontWeight: 500 }, children: line.label }), _jsx("span", { style: { fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--color-blue)", fontWeight: 600 }, children: formatCurrency(line.amount) }), bill.payments.length === 0 && (_jsx("button", { onClick: () => removeChargeMutation.mutate(line.id), title: "Waive this charge", style: { border: "none", background: "transparent", cursor: "pointer", color: "var(--color-blue)", padding: 2, display: "flex", alignItems: "center" }, children: _jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", children: _jsx("path", { d: "M18 6L6 18M6 6l12 12" }) }) }))] }, line.id))) })), bill.payments.length === 0 && discountPresets.filter((d) => d.isActive).length > 0 && (_jsxs("div", { children: [_jsxs("button", { onClick: () => { setShowDiscounts((v) => !v); setDiscountErr(""); }, style: { display: "flex", alignItems: "center", gap: 6, width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px dashed var(--color-line-strong)", background: "transparent", color: "var(--color-ink-3)", fontSize: 13, fontFamily: "inherit", cursor: "pointer" }, children: [_jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("path", { d: "M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z" }), _jsx("circle", { cx: "7", cy: "7", r: "1.5", fill: "currentColor" })] }), showDiscounts ? "Hide discounts" : `Apply discount`, _jsx("span", { style: { marginLeft: 2, fontSize: 11, background: "var(--color-surface-2)", border: "1px solid var(--color-line)", borderRadius: 20, padding: "1px 7px", color: "var(--color-ink-3)" }, children: discountPresets.filter((d) => d.isActive).length }), _jsx("svg", { width: "12", height: "12", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: { marginLeft: "auto", transform: showDiscounts ? "rotate(180deg)" : "none", transition: "transform .15s" }, children: _jsx("path", { d: "M6 9l6 6 6-6" }) })] }), showDiscounts && (() => {
                                        const orderTotal = Number(bill.subtotal) + Number(bill.taxTotal);
                                    ...
                                    borderRadius: 16, textAlign: "center",
                                        ...
                                        fontFamily: "inherit",
                                    }, children: [icon, label] }, id))) }), _jsxs("div", { children: [_jsx("div", { style: { fontSize: 12, color: "var(--color-ink-3)", marginBottom: 6 }, children: mode === "cash" ? "Cash tendered (optional)" : "Custom amount (for split)" }), _jsx("input", { value: tendered, onChange: (e) => {
                                            const raw = e.target.value.replace(/[^0-9.]/g, "");
                                            // Allow only one decimal point
                                            const parts = raw.split(".");
                                            setTendered(parts.length > 2 ? parts[0] + "." + parts.slice(1).join("") : raw);
                                        }, placeholder: "₹" + Math.ceil(remaining), style: {
