...
import { OperationTypeCards, operationTypeToSettings } from "@/components/ui/OperationTypeCards";
import { useAuthStore } from "@/stores/auth";
const DEFAULT_CREATE = { name: "", address: "", phone: "", gstin: "", timezone: "Asia/Kolkata", operationType: "full_service" };
...
export default function OwnerDashboardPage() {
    ...
    const changePwMutation = useMutation({
        ...
        onError: (e) => setChangePwErr(e.message),
    });
    function handleChangePw(e) {
        e.preventDefault();
        setChangePwErr("");
        if (changePwForm.newPassword.length < 8) {
            setChangePwErr("New password must be at least 8 characters");
            return;
        }
    ...
    function handleCreate(e) {
        ...
        if (!createForm.name.trim()) {
            setCreateErr("Outlet name is required");
            return;
        }
        ...
        if (!/^[6-9]\d{9}$/.test(createForm.phone)) {
            setCreateErr("Enter a valid 10-digit Indian mobile number");
            return;
        }
    ...
    return (_jsxs("div", { style: { minHeight: "100vh", background: "var(--color-bg)", fontFamily: "var(--font-sans)", display: "flex", flexDirection: "column" }, children: [_jsxs("header", { style: { height: 64, background: "var(--color-surface)", borderBottom: "1px solid var(--color-line)", display: "flex", alignItems: "center", padding: "0 28px", gap: 12, flexShrink: 0 }, children: [_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10 }, children: [_jsx("div", { style: { color: "var(--color-ink)", flexShrink: 0 }, children: _jsx(LogoMark, { size: 28 }) }), _jsx("span", { style: { fontSize: 16, fontWeight: 600, color: "var(--color-ink)" }, children: "InBill Owner" })] }), _jsx("div", { style: { flex: 1 } }), _jsx("span", { style: { fontSize: 13, color: "var(--color-ink-3)" }, children: dateStr }), _jsx("button", { className: "btn primary", onClick: () => setShowCreate(true), style: { fontSize: 13, padding: "0 16px", height: 34 }, children: "+ Add outlet" }), _jsx("div", { style: { width: 1, height: 22, background: "var(--color-line)", margin: "0 4px" } }), _jsx("button", { className: "btn ghost", onClick: () => setShowChangePw(true), title: "Change password", style: { padding: "0 8px", height: 34, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsx("div", { style: { width: 30, height: 30, borderRadius: "50%", background: "var(--color-accent-soft)", color: "var(--color-accent-ink)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600 }, children: ownerInitials }) }), _jsx("button", { className: "btn ghost", onClick: logout, title: "Sign out", style: { padding: "0 8px", height: 34, display: "flex", alignItems: "center", justifyContent: "center" }, children: _jsxs("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("path", { d: "M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" }), _jsx("polyline", { points: "16 17 21 12 16 7" }), _jsx("line", { x1: "21", y1: "12", x2: "9", y2: "12" })] }) })] }), _jsxs("main", { style: { padding: 32, overflow: "auto", flex: 1 }, children: [outletList.length > 0 && (_jsx("div", { style: { display: "flex", gap: 4, marginBottom: 16 }, children: ["today", "week", "month"].map((r) => (_jsx("button", { onClick: () => setRange(r), style: {
                                ...
                                gap: 14,
                                    ...
                                    return (_jsxs("div", { style: {
                                            ...
                                            }, children: [_jsx("div", { style: { width: 38, height: 38, borderRadius: 10, background: "var(--color-surface-2)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--color-ink-2)" }, children: action.icon }), _jsx("span", { style: { fontSize: 13, fontWeight: 500, color: "var(--color-ink-2)" }, children: action.label })] }, action.label))) }), outletList.length > 1 && (_jsx("p", { style: { fontSize: 11, color: "var(--color-ink-3)", marginTop: 8 }, children: "Opens first outlet. Use outlet cards above to enter a specific location." }))] }))] }))] }), showCreate && (_jsx("div", { style: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }, children: _jsxs("div", { style: { background: "var(--color-surface)", borderRadius: 18, boxShadow: "var(--shadow-3)", width: "100%", maxWidth: 500, padding: 28 }, children: [_jsx("h2", { style: { fontSize: 17, fontWeight: 700, color: "var(--color-ink)", margin: "0 0 20px" }, children: "Add Outlet" }), _jsxs("form", { onSubmit: handleCreate, style: { display: "flex", flexDirection: "column", gap: 12 }, children: [["name", "address", "phone", "gstin"].map((field) => {
                                    const labels = { name: "Name", address: "Address", phone: "Phone", gstin: "GSTIN (optional)" };
                                    const maxLengths = { name: 100, address: 500, phone: 10, gstin: 15 };
                                    const onChange = (e) => {
                                        let v = e.target.value;
                                        if (field === "phone")
                                            v = v.replace(/\D/g, "").slice(0, 10);
                                        else if (field === "gstin")
                                            v = v.toUpperCase().slice(0, 15);
                                        setCreateForm((f) => ({ ...f, [field]: v }));
                                    };
                                    return (_jsxs("div", { children: [_jsx("label", { style: { display: "block", fontSize: 12, fontWeight: 500, color: "var(--color-ink-2)", marginBottom: 5 }, children: labels[field] }), _jsx("input", { style: { width: "100%", height: 42, border: "1px solid var(--color-line-strong)", borderRadius: 10, padding: "0 14px", fontSize: 14, fontFamily: "var(--font-sans)", outline: "none", boxSizing: "border-box", color: "var(--color-ink)" }, value: createForm[field], onChange: onChange, maxLength: maxLengths[field], inputMode: field === "phone" ? "numeric" : undefined, placeholder: field === "phone" ? "10-digit mobile number" : field === "gstin" ? "e.g. 29ABCDE1234F1Z5" : undefined, required: field !== "gstin" })] }, field));
                                }), _jsxs("div", { children: [_jsx("label", { style: { display: "block", fontSize: 12, fontWeight: 500, color: "var(--color-ink-2)", marginBottom: 5 }, children: "How does this outlet operate?" }), _jsx(OperationTypeCards, { value: createForm.operationType, onChange: (operationType) => setCreateForm((f) => ({ ...f, operationType })) })] }), createErr && _jsx("p", { style: { fontSize: 13, color: "var(--color-red)", margin: 0 }, children: createErr }), _jsxs("div", { style: { display: "flex", gap: 8, paddingTop: 4 }, children: [_jsx("button", { type: "button", className: "btn ghost", onClick: () => setShowCreate(false), style: { flex: 1, justifyContent: "center", height: 40 }, children: "Cancel" }), _jsx("button", { type: "submit", className: "btn primary", disabled: createMutation.isPending, style: { flex: 1, justifyContent: "center", height: 40 }, children: createMutation.isPending ? "Creating…" : "Create" })] })] })] }) })), showChangePw && (_jsx("div", { style: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }, children: _jsx("div", { style: { background: "var(--color-surface)", borderRadius: 18, boxShadow: "var(--shadow-3)", width: "100%", maxWidth: 400, padding: 28 }, children: changePwOk ? (_jsxs("div", { style: { textAlign: "center", padding: "8px 0" }, children: [_jsx("div", { style: { width: 44, height: 44, background: "var(--color-surface-2)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }, children: _jsx("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", stroke: "var(--color-ink)", strokeWidth: "2.2", strokeLinecap: "round", strokeLinejoin: "round", children: _jsx("polyline", { points: "20 6 9 17 4 12" }) }) }), _jsx("div", { style: { fontSize: 16, fontWeight: 600, color: "var(--color-ink)", marginBottom: 6 }, children: "Password updated" }), _jsx("p", { style: { fontSize: 13, color: "var(--color-ink-3)", margin: "0 0 20px" }, children: "Your password has been changed successfully." }), _jsx("button", { className: "btn primary", onClick: closeChangePw, style: { width: "100%", height: 40, justifyContent: "center" }, children: "Done" })] })) : (_jsxs(_Fragment, { children: [_jsx("h2", { style: { fontSize: 17, fontWeight: 700, color: "var(--color-ink)", margin: "0 0 20px" }, children: "Change Password" }), _jsxs("form", { onSubmit: handleChangePw, style: { display: "flex", flexDirection: "column", gap: 12 }, children: [[
                                        { key: "currentPassword", label: "Current password" },
                                        { key: "newPassword", label: "New password" },
                                        { key: "confirm", label: "Confirm new password" },
                                    ].map(({ key, label }) => (_jsxs("div", { children: [_jsx("label", { style: { display: "block", fontSize: 12, fontWeight: 500, color: "var(--color-ink-2)", marginBottom: 5 }, children: label }), _jsx("input", { type: "password", style: { width: "100%", height: 42, border: "1px solid var(--color-line-strong)", borderRadius: 10, padding: "0 14px", fontSize: 14, fontFamily: "var(--font-sans)", outline: "none", boxSizing: "border-box", color: "var(--color-ink)" }, value: changePwForm[key], onChange: (e) => setChangePwForm((f) => ({ ...f, [key]: e.target.value })), required: true, minLength: key === "newPassword" ? 8 : undefined, maxLength: key === "currentPassword" ? undefined : 128, placeholder: key === "newPassword" ? "Min 8 characters" : "" })] }, key))), changePwErr && _jsx("p", { style: { fontSize: 13, color: "var(--color-red)", margin: 0 }, children: changePwErr }), _jsxs("div", { style: { display: "flex", gap: 8, paddingTop: 4 }, children: [_jsx("button", { type: "button", className: "btn ghost", onClick: closeChangePw, style: { flex: 1, justifyContent: "center", height: 40 }, children: "Cancel" }), _jsx("button", { type: "submit", className: "btn primary", disabled: changePwMutation.isPending, style: { flex: 1, justifyContent: "center", height: 40 }, children: changePwMutation.isPending ? "Updating…" : "Update" })] })] })] })) }) }))] }));
}
