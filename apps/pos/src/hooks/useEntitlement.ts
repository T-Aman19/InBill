...
const fallback = (feature: FeatureKey): EntitlementDecision => ({
  feature,
...
export function useEntitlements() {
  ...
  const byKey = useMemo(() => {
    const m = new Map<FeatureKey, EntitlementDecision>()
    for (const d of query.data?.features ?? []) m.set(d.feature, d)
    return m
  }, [query.data])

  const can = (feature: FeatureKey): EntitlementDecision => byKey.get(feature) ?? fallback(feature)

  return { can, isLoading: query.isLoading, refetch: query.refetch }
}

/** Convenience: the decision for a single feature. */
export function useFeature(feature: FeatureKey): EntitlementDecision {
  return useEntitlements().can(feature)
}

export const isUsable = (d: EntitlementDecision) =>
  d.state === "allowed" || d.state === "trial" || d.state === "metered"
